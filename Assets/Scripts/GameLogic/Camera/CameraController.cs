...
public class CameraController : MonoBehaviour
{
    ...
    private Texture2D defaultTexture;                                   //默认指针
    ...
    private float rotateSensitivity = 0.8f;                               // 旋转灵敏度.
    ...
    {
        if (UtilsClass.IsPointerOverUIObject())
        {
            return;
        }
        // 更新输入.
    ...
    private bool GetOverUIIsItemsPanel()
    {
        ...
        {
            if (t.gameObject.name == "ItemsPanel")
            {
                return true;
            }
        }
    ...
    private void Drag()
    {
        ...
        if (middleMouseButton)
        {
            ...
            else
            {
                ...
                // Debug.Log(m_Camera.transform.position.y);
    ...
    private void LookAround()
    {
        ...
        {
            UtilsClass.SetCursor(UtilsClass.CursorType.EYE);
            ...
            || (!leftAltKey && !rightAltKey && rightMouseButton))
        {
            ...
            if (deltaX!=0 || deltaY!=0)
                DragObject.isViewing = true; // 屏蔽拖拽物体或右键菜单
            // 相机朝前和朝后,与鼠标的滑动方向相反
        ...
        {
            UtilsClass.SetCursor(UtilsClass.CursorType.DEFAULT);
            DragObject.isViewing = false; // 接触屏蔽拖拽物体
        }
    }
    ...
    private void Magnifier()
    {
        if ((leftAltKey || rightAltKey) && rightMouseButton)
        {
            if (lastMousePosFov.Equals(Vector3.zero))
            {
                ...
                UtilsClass.SetCursor(UtilsClass.CursorType.MAGNIFIER);
            }
