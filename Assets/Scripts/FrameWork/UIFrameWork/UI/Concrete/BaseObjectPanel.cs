...
{
    public class BaseObjectPanel : BasePanel
    {
        private const string Path = "Prefabs/UI/Panel/BaseObjectPanel";
        private readonly Vector3 _position;
        private GameObject _objectGroup;
        private const float Height = 140f; // 面板高度
        private const float Width = 200f; // 面板宽度
        private const float RightContextWidth = 300f; // 右键菜单宽度
        private Emitter _emitter;

        public BaseObjectPanel(Vector3 position) : base(new UIType(Path))
        {
            _position = position;
        }
        ...
        private void CreateBase()
        {
            ...
            foreach (var toggle in toggles)
            {
                ...
                if (_emitter.baseObjects.Contains(objectName))
                    toggle.isOn = true;
                toggle.onValueChanged.AddListener(isOn =>
                {
                    if (isOn) _emitter.baseObjects.Add(objectName);
                    else _emitter.baseObjects.Remove(objectName);
                });
            }
        }
